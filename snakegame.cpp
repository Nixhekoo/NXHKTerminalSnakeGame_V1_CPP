...
#include <cstdio>
#include <sstream>
#include "NXHK\NXHK_FILESYSTEM.hpp"
...
void loadSaveFile(){
    int tempInt = NXHK_TO_INT(NXHK_LOAD(NXHK_FILEDIR("nxhktsg"), "data.txt", 1));
...
void gameEndMessage(std::string type, std::string gameEndMessage){
    ...
    if(score > highscore){
        highscore = score;
        NXHK_SAVE(NXHK_FILEDIR("nxhktsg"), "data.txt", NXHK_TO_STRING(highscore), 1);
    }
    if(type == "death"){
